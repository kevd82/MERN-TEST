...
useEffect(() => {
    ...
    <div>
        <h1>Burning Sands Character Cards</h1>
            <br/>
            <button style={{background: "Url(https://i.ibb.co/8zmvzpQ/Faeded.jpg)", borderRadius: "12px", padding: "15px 32px", backgroundSize: "100%", margin: "22px"}}><Link to = {"/createCard"}>Add new card</Link></button> 
            <br/>
                    ...
                    <td style={{outline:"1px solid black", padding: "5px"}}>
                        {
                            ...
                            <div><Link to={`/displayOne/${card._id}`}>Show {card.name}   |</Link><Link to={`/update/${card._id}`}>Update {card.name}   |</Link><button style = {{backgroundColor: "red", fontSize: "16px", fontWeight: "622", marginLeft: "5px", marginRight: "5px"}} onClick={()=>deleteCard(card._id)}>Delete {card.name}</button></div>
                            : <Link to={`/displayOne/${card._id}`}>Show {card.name}</Link>
